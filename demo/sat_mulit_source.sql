...
{%- elif source_model is iterable and source_model is not mapping -%}
    ...
    {%- if src_column_map is not none and src_column_map is mapping -%}
        ...
        {%- endfor -%}
    {%- else -%}
        {#-- No column map: check if src_payload available, else introspect --#}
        {%- if src_payload is not none and src_payload | length > 0 -%}
            {#-- src_payload provided: skip introspection, assume all sources have all payload cols --#}
            {%- for model_name in source_model -%}
                {%- do ns.source_columns.update({model_name: src_payload}) -%}
            {%- endfor -%}
        {%- else -%}
            {%- for model_name in source_model -%}
                ...
                {%- if rel is none -%}
                    {{ exceptions.raise_compiler_error("Source model '" ~ model_name ~ "' does not resolve to a valid relation. Provide src_payload or src_column_map.") }}
                {%- endif -%}
