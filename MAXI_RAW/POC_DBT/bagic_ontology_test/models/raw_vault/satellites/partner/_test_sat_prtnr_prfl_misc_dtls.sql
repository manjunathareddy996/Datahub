...
opus_partner_extn AS (
    ...
    FROM (
        SELECT * FROM {{ ref('stg_opus_partner_extn') }}
        LIMIT 1000
    ) ope
...
opus_partner_extn_hist AS (
    ...
    FROM (
        SELECT * FROM {{ ref('stg_opus_partner_extn_hist') }}
        LIMIT 1000
    ) opeh
    INNER JOIN hub_mstr hm 
...
opus_partner_hist AS (
    ...
    FROM (
        SELECT * FROM {{ ref('stg_opus_partner_hist') }}
        LIMIT 1000
    ) oph
    INNER JOIN hub_mstr hm 
...
opus_clm_ip AS (
    ...
    FROM (
        SELECT * FROM {{ ref('stg_opus_clm_interested_parties') }}
        LIMIT 1000
    ) ocip
    INNER JOIN hub_mstr hm 
...
opus_ocp_ip AS (
    ...
    FROM (
        SELECT * FROM {{ ref('stg_opus_ocp_interested_parties') }}
        LIMIT 1000
    ) ooip
    INNER JOIN hub_mstr hm 
