...
maximus_source AS (
    ...
    INNER JOIN {{ ref('stg_partner_address') }} a
        ON CAST(h.prty_id AS VARCHAR) = CAST(a.PARTY_CODE AS VARCHAR)
),
...
opus_address AS (
    ...
    INNER JOIN {{ ref('stg_opus_address') }} o
        ON CAST(h.prty_id AS VARCHAR) = CAST(o.PART_ID AS VARCHAR)
),
...
opus_address_extn AS (
    ...
    INNER JOIN {{ ref('stg_opus_address_extn') }} e
        ON CAST(h.prty_id AS VARCHAR) = CAST(e.PART_ID AS VARCHAR)
),
...
opus_address_hist AS (
    ...
    INNER JOIN {{ ref('stg_opus_address_hist') }} oh
        ON CAST(h.prty_id AS VARCHAR) = CAST(oh.PART_ID AS VARCHAR)
),
