...
-- Health Claim Package Service Details
-- Hub: hub_clm | Attributes: 10
-- Source: MAXI_HLTH_CLM_VW_DATA_CLAIM_DETAIL_PACKAGEANDSERVICE
-- SCD Type 2: Insert new row on change (append strategy)

WITH source AS (
    SELECT
        src.*
    FROM {{ ref('stg_hlth_clm_maxi_hlth_clm_vw_data_cd_packageandservice') }} src
),
