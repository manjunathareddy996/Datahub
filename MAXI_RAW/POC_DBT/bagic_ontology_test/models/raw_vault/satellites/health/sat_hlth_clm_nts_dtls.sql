...
-- Health Claim Notes Details
-- Hub: hub_clm | Attributes: 5
-- Source: MAXI_HLTH_CLM_VW_DATA_CLAIM_DETAIL_NOTES_ATTRIBUTES
-- SCD Type 2: Insert new row on change (append strategy)

WITH source AS (
    SELECT
        src.*
    FROM {{ ref('stg_hlth_clm_maxi_hlth_clm_vw_data_cd_notes_attributes') }} src
),
