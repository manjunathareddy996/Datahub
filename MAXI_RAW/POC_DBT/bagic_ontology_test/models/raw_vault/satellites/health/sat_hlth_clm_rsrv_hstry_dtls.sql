...
-- Health Claim Reserve History Details
-- Hub: hub_clm | Attributes: 8
-- Source: MAXI_HLTH_CLM_VW_DATA_CLAIM_DETAIL_CLAIM_RESERVE_HISTORY_ATTRIBUTES
-- SCD Type 2: Insert new row on change (append strategy)

WITH source AS (
    SELECT
        src.*
    FROM {{ ref('stg_hlth_clm_maxi_hlth_clm_vw_data_cd_claim_reserve_history') }} src
),
