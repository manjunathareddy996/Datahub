    ...
    if(RECURSIVE_FLAG === 1){
        ...
        `;
            ...
            var createMetaFromBaseQuery = `
                ...
                var updateMeta = `
                MERGE INTO ${TARGET_DB}.${TARGET_SCHEMA}.${SRC_TABLE}_METADATA_TEMP temp
                USING ${TARGET_DB}.${TARGET_SCHEMA}.${SRC_TABLE}_METADATA perm
                ON temp.key_path = perm.key_path
                WHEN MATCHED AND temp.key_type = ''NULL'' AND perm.key_type != ''NULL''
                THEN UPDATE SET temp.key_type = perm.key_type`;
