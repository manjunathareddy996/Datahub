...
# Constants
DAG_ID = f'MAXI_RAW_S3_TO_SF_FRO'
...
def snowflake_staging_job(**kwargs):
    ...
    FROM {stg_db}.INFORMATION_SCHEMA.TABLES 
    ...
    try:
        table_exists_result = snowflake_hook.run(sql=query_tag + check_table_qry)

        # Snowflake hook returns a list of dictionaries
        if table_exists_result and len(table_exists_result) > 0:
            table_count = table_exists_result[0].get('TABLE_COUNT', 0)
            table_exists = table_count > 0
        else:
            table_exists = False

        if table_exists:
            # Create backup clone only if table exists
            backup_qry = f"""CREATE OR REPLACE TABLE {stg_db}.{stg_schema}.{Table}_BACKUP CLONE {stg_db}.{stg_schema}.{Table};"""
            snowflake_hook.run(sql=query_tag + backup_qry)
            logs.info(f"Backup created for staging table - {stg_db}.{stg_schema}.{Table} : {Table}_BACKUP")
        else:
            logs.info(f"Table {stg_db}.{stg_schema}.{Table} does not exist. Skipping backup creation.")
        ...
        if re.search(r'already dropped', result, re.IGNORECASE):
            logs.info(f"FRO re-execution detected for table {stg_db}.{stg_schema}.{Table}")
        else:
            logs.info(f"Dropped Staging Table - {stg_db}.{stg_schema}.{Table}, Executed Query result: {result}")

    except Exception as e:
        logs.error(f"Error during backup/drop operation for table {stg_db}.{stg_schema}.{Table}")
        if truncate_qry:
            logs.error(f"Drop query attempted: {truncate_qry}")
        if backup_qry:
            logs.error(f"Backup query attempted: {backup_qry}")
        logs.error(f"Error details: {str(e)}")
        raise AirflowFailException(e)
    ...
    for x in sorted_batches:
        ...
        try:
            result = snowflake_hook.run(sql=f"USE WAREHOUSE {WAREHOUSE}; " + query_tag + copy_qry)
...
def get_previous_state(**kwargs):
    ...
    default_prv_state = '2025-01-01 05:00:00'
    ...
    """
    query2 = f"""
        DELETE FROM EMPOWER_DB.UTILS.MAXI_RAW_STATE_TABLE WHERE TABLE_NAME IN ('{table_name}');
    """
    query3 = f"""
        INSERT INTO EMPOWER_DB.UTILS.MAXI_RAW_STATE_TABLE (TABLE_NAME, LAST_MODIFIED_DATE)
        VALUES ('{table_name}', '{default_prv_state}');
    """
    try:
        snowflake_hook.run(sql=query2 + query3)
        result = snowflake_hook.get_first(query)
        if result:
            state = result[0]
    ...
    # Current_time='2025-11-19 05:29:29'

    # Convert UTC to IST (add 5:30 hours) and keep same format
    utc_str = du.UTCDateTimeNow()
    utc_dt = datetime.fromisoformat(utc_str)  # Parse ISO format
    ist_dt = utc_dt + timedelta(hours=5, minutes=30)  # Add 5:30
    Current_time = ist_dt.strftime('%Y-%m-%d %H:%M:%S')  # Convert to space format
...
def send_generic_mail(subject, body_html):
    ...
    recipient_email = [ "ashraf.shaik01@bajajgeneral.com"] #"tejas.thorat01@bajajgeneral.com",  "srinivas.gurram01@bajajgeneral.com", 
    msg = MIMEMultipart()
...
def send_report_email(table_name_local, **kwargs):
    ...
    # Get required parameters from table_info and XCom
    stg_db = table_info['staging_db']
    stg_schema = table_info['staging_schema']
    staging_table = table_info['staging_table']
    foreign_key = table_info['foreign_key']
    Mode = str(table_info['sync_type']).upper()
   # Pull start_time from XCom
    start_time = ti.xcom_pull(task_ids=f"start_ingestion_task_{mirror_table_local}", key=f"START_TIME_{mirror_table_local}_{Mode}")
    ...
    # -----------------------------
    # Query duplicate report directly from Snowflake
    # -----------------------------
    duplicate_data = None
    try:
        conn = snowflake_hook.get_conn()
        ...
        HAVING COUNT(*) > 1;
        '''
        logs.info(f"Running duplicate-report query in email task: {duplicate_query}")
        dup_df = cur.execute(duplicate_query).fetch_pandas_all()
        duplicate_data = json.loads(dup_df.to_json())
        logs.info(f"::: Duplicate report fetched successfully. Rows: {len(dup_df)}")
        
        cur.close()
        conn.close()
    except Exception as e:
        logs.error(f"::: Error while running duplicate query in email task: {e}")
        duplicate_data = {"error": str(e)}
    ...
    if isinstance(duplicate_data, dict) and 'error' in duplicate_data:
        ...
        for idx in indices:
            ...
            try:
                ...
                if (data_json and data_json.get("errorCode") == 0) or (data_json and data_json.get("response", {}).get("responseCode") == "200") or (data_json and data_json.get("proposalNumber") is not None):
                    masked_data = "*****"
            except:
    ...
    subject = "DAG RUN SUCCESSFUL: MAXI_RAW_S3_TO_SF_FRO_DAG"
    body = f"""
    <html>
    <head>
        <meta charset="UTF-8">
    ...
    <body>
        ...
        <div class="section">
            <h2>Pivot Code is not enabled in FRO Dag</h2>
        </div>
    </body>
    </html>
    """
...
# Define the DAG
dag = DAG(
    dag_id='MAXI_RAW_S3_TO_SF_FRO',
    default_args=DEFAULT_ARGS,
    description="MAXI_RAW_S3_TO_SF_FRO_DAG",
    start_date=datetime.strptime("2024-12-25 00:00:00", "%Y-%m-%d %H:%M:%S"),
    schedule_interval=None,
    is_paused_upon_creation=True,
    catchup=False,
    tags=[f'MAXIMUS_RAW', 'INGESTION', 'FRO']
)
...
for table_info_dict in json_config['table_mapping']:
    ...
    send_report_task = PythonOperator(
        ...
        op_kwargs={
            "table_name_local": table_info_dict['mirror_table'],
