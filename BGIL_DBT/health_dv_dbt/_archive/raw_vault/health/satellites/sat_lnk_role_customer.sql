...
-- Satellite: SAT_LNK_ROLE_CUSTOMER
...
-- role-type discriminator ('CUSTOMER'), NOT tied to lnk_party_role.party_role_hkey.
-- Parent: HUB_PARTY
-- Multi-active grain key: literal role type (CUSTOMER)
-- Source: {{ ref('int_health__sat_lnk_role_customer') }}. No joins in THIS load.

with source_data as (

    select parent_bk, role_type_ck, vip_indicator, record_source
    from {{ ref('int_health__sat_lnk_role_customer') }}
...
hashed as (
    ...
    select
        ...
        role_type_ck,
        vip_indicator,
        {{ dbt_utils.generate_surrogate_key(['vip_indicator']) }} as hashdiff,
        record_source,
