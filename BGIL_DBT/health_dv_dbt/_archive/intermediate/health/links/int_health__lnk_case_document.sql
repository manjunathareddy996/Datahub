-- Intermediate harmonisation view for LNK_CASE_DOCUMENT (Case Document).
...
with unioned as (
    ...
    where allocate_id is not null and omni_inward_no is not null
    ...
    where enhance_ref_id is not null and omni_inward_no is not null
    ...
    where query_ref_id is not null and omni_inward_no is not null

)

select distinct case_bk, document_bk, record_source
from unioned
