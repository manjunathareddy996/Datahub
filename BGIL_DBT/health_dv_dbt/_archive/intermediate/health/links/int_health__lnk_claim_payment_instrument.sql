-- Intermediate harmonisation view for LNK_CLAIM_PAYMENT_INSTRUMENT (Claim Payment Instrument).
...
with unioned as (
    ...
    where clid is not null and cheque_no is not null
    ...
    where claim_id is not null and cheque_no is not null
    ...
    where bjaz_claim_id is not null and cheque_no is not null

)

select distinct claim_bk, payment_instrument_bk, record_source
from unioned
