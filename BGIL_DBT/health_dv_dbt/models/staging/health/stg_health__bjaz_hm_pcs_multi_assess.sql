-- Staging model for source table BJAZ_HM_PCS_MULTI_ASSESS (Health LOB).
...
with source as (
    ...
    nullif(trim(to_varchar("PCS_SNO")), '') as pcs_sno
    from {{ source('health_raw', 'BJAZ_HM_PCS_MULTI_ASSESS') }}
