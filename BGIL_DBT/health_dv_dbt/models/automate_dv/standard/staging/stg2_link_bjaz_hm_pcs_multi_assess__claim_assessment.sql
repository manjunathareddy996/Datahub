...
-- STANDARD-MODEL per-table stage() for LNK_CLAIM_ASSESSMENT member-end 'bjaz_hm_pcs_multi_assess'.
...
{%- set yaml_metadata -%}
source_model: 'stg_health__bjaz_hm_pcs_multi_assess'
hashed_columns:
  ASSESSMENT_HKEY: 'ASSESSMENT_HKEY_NK'
  CLAIM_HKEY: 'CLAIM_HKEY_NK'
  CLAIM_ASSESSMENT_HKEY: 'CLAIM_ASSESSMENT_HKEY_NK'
derived_columns:
  ASSESSMENT_HKEY_NK: "'HUB_ASSESSMENT|' || docasess_id"
  CLAIM_HKEY_NK: "'HUB_CLAIM|' || claim_id"
  CLAIM_ASSESSMENT_HKEY_NK: "'LNK_CLAIM_ASSESSMENT|' || docasess_id || '|' || claim_id"
  LOAD_DATETIME: '!CURRENT_TIMESTAMP()'
  RECORD_SOURCE: '!BJAZ_HM_PCS_MULTI_ASSESS'
{%- endset -%}
