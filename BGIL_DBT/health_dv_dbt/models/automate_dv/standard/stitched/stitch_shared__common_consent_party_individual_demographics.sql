...
-- STANDARD-MODEL stitch view for SAT_COMMON_CONSENT, SAT_PARTY_INDIVIDUAL_DEMOGRAPHICS (HUB_PARTY grain).
...
-- downstream, in the companion stg2_shared__common_consent_party_individual_demographics.sql stage() model.
-- 2 satellites share this exact (hub, grain, table-set) signature -- built once, each
-- satellite selects only its own attribute subset downstream.
