...
-- (HUB_PARTY grain, role-special: 'agent'), table 'BJAZ_INTERMEDIARY'.
-- Reuses the exact PARTY_HKEY formula (intermediary_id) from the matching standard-model
-- stg2_rolesat_*__lnk_role_agent.sql. LICENSE_NO confirmed distinct from IRDA_LICENSE_NO (already canonical) via sample data.
