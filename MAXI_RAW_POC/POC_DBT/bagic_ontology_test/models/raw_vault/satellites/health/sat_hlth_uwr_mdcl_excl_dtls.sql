...
-- Health UWR Medical Exclusion Details
-- Hub: hub_plcy | Attributes: 14
-- Source: MAXI_HLTH_UWR_VW_DATA_MEMBERS_MEDICALRATING_EXCLUSIONS
-- SCD Type 2: Insert new row on change (append strategy)

WITH source AS (
    SELECT
        src.*
    FROM {{ ref('stg_hlth_uwr_mbr_mr_exclusions') }} src
),
